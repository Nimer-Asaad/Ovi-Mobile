...
type VariantRow = { id?: string; phoneModelId: string; colorId: string | null; variantCode: string; isActive: boolean };
type LegacyRow = { id: string; locationName: string; colorId: string | null; colorName: string | null; quantity: number };
...
export function VariantManager({ productId, brands, colors, initialVariants, legacyRows, variantStocks, inventoryReady }: { productId: string; brands: DeviceBrand[]; colors: ColorOption[]; initialVariants: VariantRow[]; legacyRows: LegacyRow[]; variantStocks: VariantStock[]; inventoryReady: boolean }) {
  ...
  const eligibleStocks = variantStocks.filter((variant) => !source?.colorId || variant.colorId === source.colorId);
  ...
  function addRow() { const firstModel = brands.flatMap((brand) => brand.models)[0]; if (firstModel) setRows((current) => [...current, { key: rowKey(), phoneModelId: firstModel.id, colorId: colors[0]?.id ?? null, variantCode: "", isActive: true }]); }

  return <div className="space-y-8">
    <form action={saveAction} className="space-y-4">
      <input type="hidden" name="variants" value={JSON.stringify(rows.map((row) => ({ id: row.id, phoneModelId: row.phoneModelId, colorId: row.colorId, variantCode: row.variantCode, isActive: row.isActive })))} />
      <div className="overflow-x-auto rounded-card border border-navy-soft"><table className="w-full text-sm"><thead className="bg-navy-soft/30 text-neutral-bg/70"><tr><th className="px-3 py-3 text-start">الماركة</th><th className="px-3 py-3 text-start">الموديل</th><th className="px-3 py-3 text-start">اللون</th><th className="px-3 py-3 text-start">SKU / Variant Code</th><th className="px-3 py-3">فعال</th><th /></tr></thead>
        <tbody className="divide-y divide-navy-soft bg-navy-surface">{rows.map((row) => { const selected = modelMap.get(row.phoneModelId); return <tr key={row.key}>
          ...
          <td className="px-3 py-2"><Select value={row.colorId ?? ""} onChange={(event) => updateRow(row.key, { colorId: event.target.value || null })}><option value="">بدون لون</option>{colors.map((color) => <option key={color.id} value={color.id}>{color.nameAr ?? color.name}</option>)}</Select></td>
        ...
        </tr>; })}{rows.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-neutral-bg/50">لا توجد Variants بعد.</td></tr>}</tbody></table></div>
    ...
    <form action={stockAction} className="space-y-4 rounded-card border border-navy-soft bg-navy-surface p-5">
      ...
      <div><h3 className="font-semibold text-neutral-bg">جرد مخزون الـVariants في المستودع الرئيسي</h3><p className="mt-1 text-sm text-neutral-bg/60">كل كمية مستقلة حسب الموديل واللون، ويُفتح الجرد بعد اعتماد توزيع الرصيد القديم.</p></div>
    ...
    <form action={allocationAction} className="space-y-4 rounded-card border border-amber-500/40 bg-navy-surface p-5">
      ...
      <Select label="مصدر الرصيد القديم" value={sourceId} onChange={(event) => { setSourceId(event.target.value); setAllocations({}); }}><option value="">اختر الرصيد</option>{legacyRows.map((row) => <option key={row.id} value={row.id}>{row.locationName} — {row.colorName ?? "غير موزع/بدون لون"} — {row.quantity} حبة</option>)}</Select>
