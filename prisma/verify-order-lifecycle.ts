  ...
  return prisma.$transaction(async (tx) => {
    ...
    for (const line of input.lines) {
      ...
      const inventory = await tx.inventoryItem.findFirstOrThrow({
        where: { productId: line.productId, locationId: input.locationId, variantId: null },
      });
...
try {
  await check("single cancellation and idempotent repeat", async () => {
    ...
    const inventory = await prisma.inventoryItem.findFirstOrThrow({ where: { productId: product.id, locationId: warehouse.id, variantId: null } });
  ...
  await check("representative return targets car stock", async () => {
    ...
    const carInventory = await prisma.inventoryItem.findFirstOrThrow({ where: { productId: product.id, locationId: repCar.id, variantId: null } });
    const warehouseInventory = await prisma.inventoryItem.findFirst({ where: { productId: product.id, locationId: warehouse.id, variantId: null } });
  ...
  await check("multi-item missing inventory rolls back atomically", async () => {
    ...
    await prisma.inventoryItem.deleteMany({ where: { productId: second.id, locationId: warehouse.id, variantId: null } });
    const before = await prisma.inventoryItem.findFirstOrThrow({ where: { productId: first.id, locationId: warehouse.id, variantId: null } });
    ...
    const after = await prisma.inventoryItem.findFirstOrThrow({ where: { productId: first.id, locationId: warehouse.id, variantId: null } });
    const state = await counts(order.id);
  ...
  await check("ledger reconciles for intact fixture pairs", async () => {
    for (const pair of touchedPairs) {
      ...
      const inventory = await prisma.inventoryItem.findFirst({ where: { productId, locationId, variantId: null } });
